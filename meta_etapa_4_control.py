...
def get_lectura_cuant():
    try:
        global right_scan, left_scan, front_scan, front_point, front_left_point
        ...
        front_left_point=lectura[100:260]
        print('lf: '+ str(np.mean(front_scan)))
        print('ll: '+ str(np.mean(left_scan)))
        print('ll: '+ str(np.mean(front_left_point)))
        sd,si,sf=0,0,0
        if np.mean(left_scan)< 1.15: si=1
        if np.mean(right_scan)< 1.0: sd=1
...
def calculate_control(robot_x, robot_y, robot_a):
    goal_x = 0 
    goal_y = 0
    cmd_vel = Twist()    
    v_max = 0.3
...
class S0(smach.State):
    def __init__(self):
        smach.State.__init__(self, outcomes=['outcome1','outcome2','outcome3','outcome4','outcome5','outcome6','outcome7','outcome8','outcome9','outcome10'])
        self.counter=0
    def execute(self,userdata):
        si,sd,sf=get_lectura_cuant()
        msg_cmd_vel=Twist()
        tole_i, tole_f = 3.5, 2
        global xp, yp, tol_1, tol_2, xr, yr, left_scan, rigth_scan, theta_R, radio_xp, radio_yp, angle_to_goal, angle_to_goal1, inc_x, inc_y, front_left_point
        ...
        '''
        xr, yr, theta = get_robot_pose(listener) 
        ...
        if (np.mean(left_scan)<1 and tol_2 == 0):
        ...
        if (np.mean(right_scan)<0.8 and tol_2 == 0):
            ...
            move_base(0.0,0.0,0.12*np.pi,0.2)
            #return 'outcome1'
        '''if (np.mean(front_point)<0.3 and tol_2 == 0):
            print('base apegada frente')
            move_base(0.0,0.0,-0.12*np.pi,0.2)
            #return 'outcome1'
        if (np.mean(front_left_point) < 1 and tol_2 == 0):
            print('base apegada frente izquierda')
        ...
        if (tol_2 == 1):
            ...
            if (si==0 and sd==0 and sf==0): return 'outcome10'
            return 'outcome10'
...
class S1(smach.State):
    ...
    def execute(self,userdata):
        ...
        if (ini==0):
            ...
            xp, yp, robot_a = get_robot_pose(listener)
            ini = 1
        if (tol_2 == 0):
...
class S9(smach.State):
    ...
    def execute(self,userdata):
        ...
        msg_cmd_vel = calculate_control(robot_x, robot_y, robot_a)
        pub_cmd_vel.publish(msg_cmd_vel)
        return 'outcome1'
        
def main():
    global pub_cmd_vel, detect,ini, listener
...
if __name__ == '__main__':
    ...
    with sm:
        smach.StateMachine.add("s_0",   S0(),  transitions = {'outcome1':'s_0', 'outcome2':'s_1','outcome3':'s_2','outcome4':'s_3','outcome5':'s_4', 'outcome6':'s_5','outcome7':'s_6','outcome8':'s_7','outcome9':'s_8','outcome10':'s_9'})
        ...
        smach.StateMachine.add("s_9",   S9(),  transitions = {'outcome1':'s_0','outcome2':'END'})
    try:
        main()
